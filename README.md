...
Merlin can identify a bird. BirdSong Bridge helps you **remember** one — where you were, what it meant, and what it might sound like set to music.
...
And even when the ID is right, identifying isn't *remembering*. BirdSong Bridge is built to make the identification honest enough to trust, then keep the moment — organised by place, and turnable into music.
...
A fixed-threshold `if confidence > 0.8, auto-accept` rule can't reason about plausibility — it has no way to know a candidate species has never been recorded within 25 km of where you're standing. An agent that can call a regional-checklist tool and weigh that against the raw acoustic score can. That's the whole argument for putting an LLM in this one step, and it's why `verify_agent` exists as a distinct node rather than a `confidence > X` if-statement (see `app/agent.py`).
...
│   └── requirements.txt
├── frontend/
│   ├── index.html           # the design, wired to the real backend
│   ├── assets/
│   └── Dockerfile
...
> **Windows note:** run the backend from the project root or the `backend/` folder with `uvicorn main:api --port 8080`. Keys are read from `.env` via `python-dotenv` (already a dependency) — no need to export them each time.

## API keys

Every key is free. See the build guide for exact signup steps.
...
Place Memory currently produces a **soundscape brief** — a score a musician (or a music model) can compose from — not generated audio. The brief has been validated by hand against a music-generation model; wiring that generation step directly into the app is the next milestone.
